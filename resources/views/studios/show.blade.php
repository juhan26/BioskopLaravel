            ...
            <div>
                <label for="seat_id" class="inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="seat_id" id="seat_id"
                        value="{{ $seat->id }}"
                        class="form-checkbox h-4 w-4 text-sky-700 border-gray-300 rounded focus:ring-sky-700" disabled checked>
                    <span class="lg:ml-2 {{ $seat->is_reserved ? 'text-red-500' : 'text-gray-900 dark:text-white' }}">
                        {{ $seat->seat_number }}
                    </span>
                </label>
            </div>
